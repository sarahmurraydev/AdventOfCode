...
fun getMode(data: List<Int>): Int {
    val dataGroupedByCount = data.groupingBy { it }.eachCount()
    val mode = dataGroupedByCount.maxByOrNull { it.value }?.key
    return if (mode != null) {
        mode
    } else {
...
fun getMedian(data: List<Int>): Int {
    val dataGroupedByCount = data.sorted()
